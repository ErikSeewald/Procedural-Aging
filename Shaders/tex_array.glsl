...
vec4 layer0(ivec2 pos, ivec2 dims, float age)
{
	return vec4(pos, age/20.0, 1.0);
}

vec4 layer1(ivec2 pos, ivec2 dims, float age)
{
	float g = fract(3 + float(pos.x ^ pos.y) * 0.001);
	return vec4(g, g, g, 1.0);
}
vec4 layer2(ivec2 pos, ivec2 dims, float age)
{
	return vec4(vec2(pos) / vec2(dims), dims.x, 1.0);
}
...
void main()
{
	...
	switch (layer)
	{
		case 0: color = layer0(pos, dims, age); break;
